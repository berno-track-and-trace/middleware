...
const printerTemplate = {
    ...
    template2: function (details, fileName) {
        ...
        const BN = printer.createModuleText(`BN  ${details.BN}`,  true,  10,  154+5,  0,  22,  0,  "Arial"); 
        const MD = printer.createModuleText(`MD  ${details.MD}`,  true,  10,  178+5,  0,  22,  0,  "Arial");
        const ED = printer.createModuleText(`ED  ${details.ED}`,  true,  10,  206+5,  0,  22,  0,  "Arial");
        const HET = printer.createModuleText(`HET Rp ${details.HET} / ${details.UNIT.toUpperCase()}`,  true,  10,  234+3,  0,  22,  0,  "Arial");
    ...
    template3: function (details, fileName) {
        ...
        const modules= [ BPOM,QR, field,BN, MD, ED, HET]
        const msg = printer.createMsg(modules, fileName);
        console.log("[PrinterTamplate] tamplate3 is chosen")
        return msg;
    }
};
